...
def record_audio():
    ...
    for i in range(0, n+1):
        ...
        fileName = "C:\\Users\\sinha\\Desktop\\Voice-Biometric\\data\\akshat" + str(i) + ".wav"
...
def createMfccCsv():
    ...
    for i in range(0, 21):
        ...
        lst = list(mfccs)
        df.loc[len(df)] = lst
    df.to_csv("C:\\Users\\sinha\\Desktop\\Voice-Biometric\\data\\akshat\\akshat.csv") #dont forget to change the paths (there are literally many paths in the project)


# Append all individual data to complete data.
def appendIndividualToCompleteCSV():
    df1 = pd.read_csv("C:\\Users\\sinha\\Desktop\\Voice-Biometric\\data\\complete_data.csv") #read completedata.csv
    df2 = pd.read_csv("C:\\Users\\sinha\\Desktop\\Voice-Biometric\\data\\akshat\\akshat.csv") #read speaker_name.csv
    df1 = df1._append(df2, ignore_index=True) #append df2 into df1
    df1.to_csv("C:\\Users\\sinha\\Desktop\\Voice-Biometric\\data\\akshat\\complete_data.csv") # save df1 in complete_data.csv (!! Note that the csv file should not be open in excel, that will give error).
